...
func (r *DomainRunner) ExecuteDomainScan(ctx context.Context, scanner scannerDomain.ScannerDomain, scanJobID int64) error {
	...
	return r.processComputersToAssets(scanCtx, computers, scanJobID, scanner.Domain)
}
...
func (r *DomainRunner) processComputersToAssets(ctx context.Context, computers []ComputerEntry, scanJobID int64, scannerDomain string) error {
	logger.InfoContext(ctx, "[DomainScanner] Processing %d computers", len(computers))
	...
	for i, computer := range computers {
		...
		// Create asset domain object with the scanner's domain
		...
		// Store the asset
		logger.InfoContext(ctx, "[DomainScanner] Creating asset for computer: %s with domain: %s", name, scannerDomain)
		...
		logger.InfoContext(ctx, "[DomainScanner] Successfully processed computer %s (Asset ID: %s) discovered by DOMAIN with domain: %s", name, assetID, scannerDomain)
		totalAssets++
	}
